...
from utils import extract_CAM_info_from_pdf, TextLogger
import json
import logging
...
class Main():
    def __init__(self):
        ...
        self.save_path = os.path.join(base_dir, r"data\teste")
    ...
    def create_storage_selector_frame(self):
        ...
        self.selector_frame.grid_rowconfigure([0,1,2], weight=1)
        ...
        # Label for the dropdown
        selector_label = tk.Label(self.selector_frame, text="CAM File:", font=("Arial", 11))
        selector_label.grid(row=2, column=0, padx=(0, 0), sticky="e")

        # Entry for CAM file
        self.CAM_file_button = Button(self.selector_frame, text="Browse", width=10, height=1, state="normal", command=self.select_file)
        self.CAM_file_button.grid(row=2, column=1, padx=0, pady=0, sticky="w")
    ...
    def select_file(self):
        """Open a file dialog to select a CAM file."""
        from tkinter import filedialog
        file_path = filedialog.askopenfilename(filetypes=[("PDF files", "*.pdf")])
        if file_path:
            self.CAM_file_button.config(text=os.path.basename(file_path))
            # Here you can add code to process the selected file if needed
            extract_CAM_info_from_pdf(file_path)
            #print(f"Selected CAM file: {file_path}")         
    ...
    def create_debug_frame(self):
        ...
        self.status_label = tk.Label(self.debug_frame, text="Press Start to start acquisition", font=("Arial", 12, "bold"), fg="white", bg="red",padx=900, pady=50)
    ...
    def create_sensor_selection_frame(self):
        ...
        label = Label(selection_frame, text="Select Sensors:", font=("Helvetica", 12))
        label.grid(row=0, column=0, sticky="w", pady=(0, 0))

        self.daq_checkbox = Checkbutton(
            selection_frame, text="DAQ", variable=self.use_daq, font=("Helvetica", 11))
        self.daq_checkbox.grid(row=1, column=0, sticky="w")

        self.wifi_checkbox = Checkbutton(
            selection_frame, text="WiFi ACCEL", variable=self.use_wifi, font=("Helvetica", 11), state= "normal")
        self.wifi_checkbox.grid(row=2, column=0, sticky="w")

        self.mic_checkbox = Checkbutton(
            selection_frame, text="MIC", variable=self.use_mic, font=("Helvetica", 11), state= "disabled")
        self.mic_checkbox.grid(row=3, column=0, sticky="w")

        self.mqtt_checkbox = Checkbutton(
            selection_frame, text="OTHER", variable=self.use_mqtt, font=("Helvetica", 11), state= "disabled")
    ...
    def setup_readers(self):
        ...
        if self.use_daq.get():
            ...
            self.reader_DAQ = USB_DAQ(sampling_rate=self.sampling_rate,
                                    selected_sensors=self.selected_sensors_daq,
                                    data_queue = self.daq_data_queue,
                                    running_flag=self.is_running,
                                    handler=self.telegraf_handler,
                                    auto = self.auto, error_callback=self.handle_daq_error,
                                    timed=self.timed, 
                                    time_limit=self.acquisition_duration,
                                    start_callback=self.on_acquisition_started,
                                    stop_callback=self.on_acquisition_stopped
                                )
    ...
    def start_acquisition(self):
        ...
        self.log_file_path = os.path.join(base_dir, 'logs', f'{timestamp}.txt')
        ...
        logging.info(f"Selected Sensors: {self.header}")
        ...
        if self.auto:
            self.status_label.config(text="Waiting for starting thrigger", bg="Blue")

        #if self.timed:
        #    self.auto_stop_thread = threading.Thread(target=self.auto_stop_timer, daemon=True)
    ...
    def stop_acquisition(self):
        ...
        logging.info(f"Total time start/stop: {total_time:.2f} s")
    ...
    def auto_stop_timer(self):
        """Automatically stop acquisition after a fixed duration."""
        time.sleep(self.acquisition_duration)
        if self.is_running.is_set():  # Check again in case stop was manual
            self.stop_acquisition()
    ...
    def on_acquisition_stopped(self):
        self.status_label.config(text="Waiting to Acquiring\n Press Restart to restart the program", bg="blue")
        self.restart_button.config(state="normal")
        self.start_button.config(state="normal")
        self.stop_button.config(state="disabled")
